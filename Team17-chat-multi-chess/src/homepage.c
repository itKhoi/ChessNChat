//created by samuel wong and hayley sperber
...
//new code
int registerflagup(int regflag){
	regflag=1;
	return regflag;
}
//new code
...
//new code
    g_signal_connect_swapped (button2, "clicked", regflag(regflag), window);
//new code
...
//new code
    //pseudo code: if button2 clicked, 
    if(regflag==1){
//new code
...
//register gui 
	...
	/* confirm password text entry */
        ...
        check = gtk_check_button_ne w_with_label ("Visible");
        gtk_box_pack_start (GTK_BOX (hbox), check, TRUE, TRUE, 0);
        g_signal_connect (check, "toggled", G_CALLBACK (entry_toggle_visibility), entry);
        gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (check), TRUE);
        gtk_widget_show (check);*/

	button_r = gtk_button_new_with_label("Register Now");
        g_signal_connect_swapped (button_r, "clicked", G_CALLBACK (gtk_widget_destroy), window);
        gtk_box_pack_start (GTK_BOX (vbox), button_r, TRUE, TRUE, 0);
        gtk_widget_set_can_default (button_r, TRUE);
        gtk_widget_grab_default (button_r);
        gtk_widget_show (button_r);
